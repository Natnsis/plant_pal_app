...
class UserProfile {
  ...
  factory UserProfile.fromJson(Map<String, dynamic> j) => UserProfile(
        id: idOf(j),
        fullName: asString(pick(j, ['full_name', 'fullName', 'name']), 'PlantPal user'),
        ...
        tasksDone: asInt(pick(j, ['total_task_done', 'totalTaskDone', 'tasks_done'])),
        journalEntries: asInt(pick(j, [
...
PlantHealth plantHealthFrom(Object? v) =>
    asString(v) == 'needs_attention' ? PlantHealth.needsAttention : PlantHealth.good;
...
class Species {
  ...
  factory Species.fromJson(Map<String, dynamic> j) => Species(
        id: idOf(j),
        commonName: asString(pick(j, ['common_name', 'commonName']), 'Unknown plant'),
        scientificName: asString(pick(j, ['scientific_name', 'scientificName'])),
        family: asString(pick(j, ['family'])),
        difficulty: difficultyFrom(pick(j, ['difficulty_level', 'difficultyLevel'])),
...
class CarePlan {
  ...
  factory CarePlan.fromJson(Map<String, dynamic> j) => CarePlan(
        wateringFrequencyDays:
            asInt(pick(j, ['watering_frequency_days', 'wateringFrequencyDays'])),
        ...
        lightRequirement:
            asString(pick(j, ['light_requirement', 'lightRequirement'])),
        humidityRequirement:
            asString(pick(j, ['humidity_requirement', 'humidityRequirement'])),
        soilType: asString(pick(j, ['soil_type', 'soilType'])),
        fertilizerType: asString(pick(j, ['fertilizer_type', 'fertilizerType'])),
        pruningFrequency:
            asString(pick(j, ['pruning_frequency', 'pruningFrequency'])),
        repottingFrequency:
            asString(pick(j, ['repotting_frequency', 'repottingFrequency'])),
...
class ActivityLog {
  ...
  factory ActivityLog.fromJson(Map<String, dynamic> j) => ActivityLog(
        ...
        loggedDate: asDate(pick(j, ['logged_date', 'loggedDate'])) ?? createdAtOf(j),
      );
}
...
class Reminder {
  ...
  String get title {
    ...
    return plantName.isEmpty ? verb : '$verb $plantName';
  }

  Reminder copyWith({bool? isCompleted, int? snoozeCount}) => Reminder(
        id: id,
        taskType: taskType,
...
class CommunityPost {
  ...
  factory CommunityPost.fromJson(Map<String, dynamic> j) => CommunityPost(
        ...
        authorInitials: asString(pick(j, ['author_initials', 'authorInitials']), '··'),
...
class Forecast {
  const Forecast({required this.current, required this.hourly, required this.daily});
...
class CurrentWeather {
  const CurrentWeather({required this.temp, required this.humidity, required this.icon});
  final int temp;
  final int humidity;
  final String icon;
...
class NotificationSettings {
  ...
  factory NotificationSettings.fromJson(Map<String, dynamic> j) =>
      NotificationSettings(
        notificationEnabled:
            asBool(pick(j, ['notification_enabled', 'notificationEnabled']), true),
        dailySummaryEnabled:
            asBool(pick(j, ['daily_summary_enabled', 'dailySummaryEnabled'])),
        soundAlertEnabled:
            asBool(pick(j, ['sound_alert_enabled', 'soundAlertEnabled'])),
        vibrationEnabled:
            asBool(pick(j, ['vibration_enabled', 'vibrationEnabled']), true),
        preferredTime: asDate(
            pick(j, ['preferred_notification_time', 'preferredNotificationTime'])),
        snoozeMinutes: asInt(
            pick(j, ['default_snooze_duration_minute', 'defaultSnoozeDurationMinute']),
            15),
      );
}
...
class ScanResult {
  ...
  int get id => idOf(_scan) == 0 ? asInt(pick(raw, ['id', 'scan_id', 'ID'])) : idOf(_scan);
  double get confidence =>
      asDouble(pick(raw, ['confidence_score', 'confidence', 'confidenceScore'])) == 0
          ? asDouble(pick(_scan, ['confidence_score', 'confidenceScore']))
          : asDouble(pick(raw, ['confidence_score', 'confidence', 'confidenceScore']));
  int get confidencePercent =>
      (confidence <= 1 ? confidence * 100 : confidence).round();
  String get commonName => asString(pick(raw, [
        'common_name',
        'commonName',
        'name',
      ]), 'Identified plant');
  String get scientificName =>
      asString(pick(raw, ['scientific_name', 'scientificName', 'species']));
  String get imageUrl =>
      asString(pick(raw, ['captured_image_url', 'image_url', 'imageUrl']));
...
class DiagnosisMessage {
  ...
  factory DiagnosisMessage.fromJson(Map<String, dynamic> j) {
    final role = asString(pick(j, ['role', 'sender', 'author'])).toLowerCase();
    final fromUser = asBool(pick(j, ['is_user', 'isUser', 'me'])) ||
        role == 'user' ||
        role == 'me';
    return DiagnosisMessage(
      fromUser: fromUser,
      text: asString(pick(j, ['message', 'text', 'content'])),
    );
  }
}

class DiagnosisSession {
  DiagnosisSession({required this.id, required this.messages, required this.raw});
  ...
  factory DiagnosisSession.fromJson(Map<String, dynamic> j) {
    ...
    final rawMsgs = pick(j, ['messages', 'chat', 'history']);
